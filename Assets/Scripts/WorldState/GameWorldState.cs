...
namespace SurfaceRiseOfHumanity.WorldState
{
    ...
    [Serializable] public sealed class PlayerState { public Vector3 position; public Vector3 rotationEuler; public string currentChunk; public int level = 1; public int experience; public int skillPoints; public int health = 100; public int maxHealth = 100; public int armor; public float energy = 100f; public float maxEnergy = 100f; public float stamina = 100f; public float maxStamina = 100f; public string currentWeapon = "AR7_SURVIVOR_RIFLE"; public int magazine = 30; public int reserveAmmo = 120; }
    ...
    [Serializable] public sealed class DiscoveryState { public List<IdRecord> discovered = new List<IdRecord>(); public bool Has(string id) => discovered.Exists(x => x.id == id); public void Add(string id) { if (!string.IsNullOrWhiteSpace(id) && !Has(id)) discovered.Add(new IdRecord(id)); } }
    [Serializable] public sealed class ScannerState { public List<string> scannedObjectIds = new List<string>(); public int rawMachineData; public bool scannerRangeUpgrade; public bool scanSpeedUpgrade; public void AddScan(string id) { if (!scannedObjectIds.Contains(id)) scannedObjectIds.Add(id); } }
    [Serializable] public sealed class MachineKnowledgeState { public List<MachineKnowledgeRecord> machines = new List<MachineKnowledgeRecord>(); public MachineKnowledgeRecord GetOrCreate(string id) { MachineKnowledgeRecord value = machines.Find(x => x.machineId == id); if (value == null) { value = new MachineKnowledgeRecord { machineId = id }; machines.Add(value); } return value; } }
    ...
    [Serializable] public sealed class GameWorldState
    {
        ...
        public DiscoveryState discoveries = new DiscoveryState();
        public ScannerState scanner = new ScannerState();
        public MachineKnowledgeState machineKnowledge = new MachineKnowledgeState();
        public InventoryState inventory = new InventoryState();
        public EquipmentState equipment = new EquipmentState();
        public EncounterState encounters = new EncounterState();
        public WorldEventState events = new WorldEventState();
        ...
        public static GameWorldState CreateNew()
        {
            ...
            state.metadata.saveId = Guid.NewGuid().ToString("N");
            state.metadata.createdUtc = DateTime.UtcNow.ToString("O");
            state.world.regions.Add(new RegionState { regionId = WorldRegionId.TheDeep, unlocked = true, discovered = true });
            state.world.regions.Add(new RegionState { regionId = WorldRegionId.TheAscent });
            state.world.regions.Add(new RegionState { regionId = WorldRegionId.GreenZone });
            state.inventory.items.Add(new InventoryRecord { itemId = "AR7_SURVIVOR_RIFLE", category = InventoryCategory.Weapons, quantity = 1 });
            state.inventory.items.Add(new InventoryRecord { itemId = "BASIC_SURVIVAL_SUIT", category = InventoryCategory.Armor, quantity = 1 });
            return state;
        }
    }
}
